...
class Corpus(object):
    ...
    def tagged_corpus(self):
        ...
        return [
            TaggedDocument(words=text, tags=tags) for tags, text in self.corpus.items()
        ]
    ...
    def sentence_tranformer(self, model="sentence-transformers/all-mpnet-base-v2"):
        # model can be found here: https://huggingface.co/sentence-transformers
        from sentence_transformers import SentenceTransformer
...
def vectorization_main(model):
    ...
    print(f"starting vectorization for {model} on {date}")
